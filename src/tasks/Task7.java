...
public class Task7 implements Task {
    ...
    private Set<String> vacancyNames(Collection<Company> companies) {
        ...
        return companies.stream() // создаем стрим компаний
                .flatMap(company -> company.getVacancies().stream()) // превращаем в стрим вакансий
                .map(Vacancy::getTitle) // мапим в названия вакансий
